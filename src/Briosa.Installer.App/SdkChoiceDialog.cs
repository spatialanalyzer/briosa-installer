...
public sealed class SdkChoiceDialog : Window
{
    ...
    public SdkChoiceDialog(IEnumerable<SdkObservation> installations, string current)
    {
        ...
        var choices = new ComboBox { Name = "SdkInstallationChoice", ItemsSource = installations, DisplayMemberPath = "Version", MinWidth = 240 };
        ...
        choices.SelectionChanged += (_, _) => { SelectedInstallation = choices.SelectedItem as SdkObservation; path.Text = SelectedInstallation?.Location ?? "Choose an installed SA release."; review.IsEnabled = SelectedInstallation is not null; };
